...
(define interpret-function
  (lambda (statement environment)
      (insert (cadr statement) (cons (caddr statement) (cadddr statement)) environment)))
...
(define interpret-funcall
  (lambda (statement environment throw)
    (call/cc
      (lambda (return)
        (interpret-statement-list (cdr (lookup (cadr statement) environment)) (add-binding (car (lookup (cadr statement) environment)) (cddr statement) environment (push-frame (pop-frame environment)) throw) return
                                  (lambda (env) (myerror "Break used outside of loop")) (lambda (env) (myerror "Continue used outside of loop"))
                                  throw)))))
