...
(define m_return
  (lambda (expr s)
    (cond
      ...
      ((and (pair? expr) (member? (prefix expr) boolop)) (m_bool expr s))
...
(define m_value
  (lambda (expr s)
    (cond
      ...
      ((and (eq? '- (operator expr)) (not (secondarg? expr))) (- 0 (m_value (operand1 expr) s)))
...
(define m_state_ifwhile
  (lambda (expr s)
    (cond
      ((eq? (operator expr) 'while) (if (m_bool (execcond expr) s) (m_state_ifwhile expr (m_state (list (body expr)) s)) s))
      ((isIfElse expr) (if (m_bool (execcond expr) s) (m_state (list (body expr)) s) (m_state (elsebody expr) s)))    ; todo the and clause         todo also change the "variable-like" use of (define **** '())
