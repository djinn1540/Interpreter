...
(define m_return
  (lambda (expr s)
    (cond
      ...
      ((number? expr) expr)
      ((boolean? expr) expr)
      ((and (pair? expr) (member? (prefix expr) arithop)) (m_value expr s))
      ((and (pair? expr) (member? (stmttype expr) boolop)) (m_bool expr s))
      (else (m_value expr s)))))
...
(define else (lambda (lis) (car (cdddr))))

;
(define m_state_ifwhile
  (lambda (expr s)
    (cond
      ((eq? (operator expr) 'while) (if (m_bool (cond expr) s) (m_state_ifwhile expr (m_state (body expr) s))
      ((eq? (operator expr) 'if) (if (m_bool (cond expr) s) (m_state (body expr) s) (m_state (else expr) s))))))))                       
...
(define comparison_op
  (lambda (expr s)
    ...
    (cond 
          ((null? (car (cddr expr))) #f)
          ;use m_value in case a list with arithmetic operators are given
          ((eq? '>= (car expr)) (>= (m_value (operand1 expr) s) (m_value (operand2 expr) s)))
          ((eq? '<= (car expr)) (<= (m_value (operand1 expr) s) (m_value (operand2 expr) s)))
          ((eq? '> (car expr)) (> (m_value (operand1 expr) s) (m_value (operand2 expr) s)))
          ((eq? '< (car expr)) (< (m_value (operand1 expr) s) (m_value (operand2 expr) s)))
          (else #f))))
      ...
      (cond
        ...
        ((or(list? (operand1 expr)) (list? (operand2 expr)))
         (if (or(check_arithmetic (operand1 expr)) (check_arithmetic (operand2 expr)))
             (eqv? (m_value (operand1 expr) s) (m_value (operand2 expr) s))
             (eqv? (m_bool(operand1 expr) s) (m_bool (operand2 expr) s))))
         (else (eq? (operand1 expr) (operand2 expr))))))
    ...
    (lambda (expr s)
      (cond
        ((or(list? (operand1 expr)) (list? (operand2 expr)))
         ...
         (else (not(eq? (operand1 expr) (operand2 expr)))))))
